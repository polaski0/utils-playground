...
describe("string schema validator", () => {
    ...
    it("should validate string `foo` to true", () => {
        const stringSchema = s.string().matches(new RegExp(/foo/))
    ...
    it("should validate string `bar` to false", () => {
        const stringSchema = s.string().matches(new RegExp(/foo/))
        const result = stringSchema.validate("bar");
        expect(result.valid).toBe(false);
    })
})
